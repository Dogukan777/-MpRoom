<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Odalarr.aspx.cs" Inherits="MedipolChat2.Odalar" %>
...
<head runat="server">
 ...
 <script>
      $(function () {
          ...
          var $odaAdi = $('#lblOdaAdi');
          scrollBottom();
          $message.focus();

          chat.client.newOdaSend = function (name, message, odaAdi) {
              if (odaAdi == $odaAdi.text()) {
                  if (name == $username.val() && message.trim() != "") {
                      $messages.append('<div class="IBox"><div class="ImessageBox" >' + '<div class="messageUst"><span class="ImessageName">' + name + '</span>' + '<span class="ImessageDate">' + strDate + '</span></div>' + '' + message + '</div></div>');
                      scrollBottom();
                     
              }
                  else if (message.trim() != "") {
                      $messages.append('<div class="Box"><div class="messageBox">' + '<div class="messageUst"><span class="messageName">' + name + '</span>' + '<span class="messageDate">' + strDate + '</span></div>' + '' + message + '</div></div>');
                      scrollBottom();
          ...
          $.connection.hub.start().done(function () {
              $('#btnSendMessage').click(function () {
                  if ($message.val().trim() != "") {
                      chat.server.newOdaMesajEkle($username.val(), $message.val(), $odaAdi.text());
                  }
                  chat.server.newOdaSend($username.val(), $message.val(), $odaAdi.text());
